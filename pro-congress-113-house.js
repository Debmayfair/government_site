...
/*function getInnerTextfromJSON(data){
    document.getElementById("house_data").innerHTML = JSON.stringify(data.results,null,2);
}
getInnerTextfromJSON(data)*/

var members = data.results[0].members;
var filteredMembers = [];
var dataTabla = document.getElementById("house_data");
...
function getDataHeader(){
    ...
    header1.textContent = "Senators";
    headerRow.appendChild(header1);
    
    var header2 = document.createElement("th");
    header2.textContent = "Party";
    headerRow.appendChild(header2);
    
    var header3 = document.createElement("th");
    header3.textContent = "State";
    headerRow.appendChild(header3);
    
    var header4 = document.createElement("th");
    header4.textContent = "Years in Office";
    headerRow.appendChild(header4);
    
    var header5 = document.createElement("th");
    header5.textContent = "% Votes w/ Party";
    headerRow.appendChild(header5);
    
    dataTabla.appendChild(headerRow);   

}

getDataHeader()

function getDataBody(members){
    var dataTabla = document.getElementById("house_data");
    var tableBody = document.createElement("tbody");
        for (var i = 0; i<members.length; i++){
            ...
            if (members[i].middle_name == null){
                ...
                var stringinDatas = members[i].middle_name;
            }
			var senateNames = members[i].first_name +" " + stringinDatas + " " + members[i].last_name;
        
            var anchor = document.createElement("a");
            anchor.setAttribute("href", members[i].url);
            anchor.textContent = senateNames;
            column1.appendChild(anchor); 
  
        
            var column2 = document.createElement("td");
            var senateParty = members[i].party;
            column2.textContent = senateParty;
       
            var column3 = document.createElement("td");
            var senateState = members[i].state;
            column3.textContent = senateState;
        
            var column4 = document.createElement("td");
            var officeYears = members[i].seniority;
            column4.textContent = officeYears;
            
            var column5 = document.createElement("td");
            var senateVotes = members[i].votes_with_party_pct + "%"
            column5.textContent = senateVotes;
    ...
    dataTabla.appendChild(tableBody);
}
}

getDataBody(members)

/* Elimina el body al hacer el check*/
function removeBody(){
    var table = document.getElementById("house_data");
    table.removeChild(table.childNodes[1]);
    /*Queremos que se elimine el segundo hijo, el primero es el header, al eliminar el segundo, eliminas el body de la tabla */   
}

function myFunction(){
    var filteredMembers = [];
    var checkboxR = document.getElementById("oncheckR");
    var checkboxD = document.getElementById("oncheckD");
    var checkboxI = document.getElementById ("oncheckI");

    for (var i = 0; i< members.length; i++){
    var partyKey = members[i].party;
        
    if(checkboxR.checked == true && checkboxR.value === partyKey){
        filteredMembers.push(members[i]);        
    }
        
    else if(checkboxD.checked == true && checkboxD.value === partyKey){
        filteredMembers.push(members[i]);
    }
        
    else if(checkboxI.checked == true && checkboxI.value === partyKey){
        filteredMembers.push(members[i]);
    }
  }
            
    if (checkboxR.checked == false && checkboxD.checked == false && checkboxI.checked == false){
        var table = document.getElementById("house_data");
        var div = document.createElement("div");
        div.textContent = "No results found for your search criteria";
        table.appendChild(div);
    }
    removeBody()
    getDataBody(filteredMembers);  
}

var state = document.getElementById("house-data");
...
function stateFilter(members){
    ...
    var senatorState = members[x].state;

    if (state.value == "All")
        filteredState.push(members[x]);
    else if (state.value === senatorState){
        filteredState.push(members[x]);
    }
    
  }
    removeBody()
    getDataBody(filteredState)
}

/* No se llama a myFunction en el código porque la llamamos con el onclick
myFunction(filteredMembers)*/
