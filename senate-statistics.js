...
var membersRepublicanParty = [];
var membersDemocratParty = [];
var membersIndependentParty = [];

function senatePartyLength(partyLength){
   
    for(var i = 0; i<partyLength.length; i++){
        var party = partyLength[i].party;
        
        if(party == "D"){
            membersDemocratParty.push(partyLength[i]);
            
        }
        
        else if (party == "R"){
            membersRepublicanParty.push(partyLength[i]);
       
        }
        else if ( party == "I"){
            membersIndependentParty.push(partyLength[i]);
...
senatePartyLength(senateMembers)

console.log(membersDemocratParty);

function partyAverage(members){
 var total = 0;
    for (var i = 0; i<members.length; i++){
        var totalPercent = members[i].votes_with_party_pct;
        total += totalPercent; /*+= sintaxis es igual a total = total + totalPercent*/
  	}
	var totalVotes = total / members.length;
	var totalVotes = Math.round(totalVotes*100)/100;
	return totalVotes;
}



//    for (var i = 0; i<membersDemocratParty.length; i++);{
//        var democratsPercent = senateMembers[i].votes_with_party_pct;
//        var DemocratsVotes = (total+democratsPercent) / membersDemocratParty.length;
//    }
//    for (var i = 0; i<membersRepublicanParty.length; i++);{
//        var republicansPercent = senateMembers[i].votes_with_party_pct;
//        var RepublicanVotes = (total+republicansPercent) / membersRepublicanParty.length;
//    }
//   
...
function leastLoyal(){
    var senateMembers = data.results[0].members;
    var sortedMembersPercent = []
        for (var i = 0; i<senateMembers.length; i++ ){
            var totalPercent = senateMembers[i].votes_with_party_pct;
            sortedMembersPercent.push(totalPercent);
             sortedMembersPercent.sort(function(a, b){
                return a - b});
            var tenPercentValue = (sortedMembersPercent.length*10)/100;
            var leastLoyal = sortedMembersPercent.slice(0, tenPercentValue);
             /*conseguir los miembros, no solo el valor*/
...
leastLoyal()
*/
/*console.log(JSON.stringify(attendance_statics["% Voted w/ Party"]));*/




var statistics = { 
					"NumberOfDemocrats": membersDemocratParty.length,
                    "NumberOfRepublicans": membersRepublicanParty.length,
                    "NumberOfIndependents": membersIndependentParty.length,
					"TotalParty": senateMembers.length,
					"AverageVoteWPartyDemocrats": partyAverage(membersDemocratParty),
					"AverageVoteWPartyRepublicans":partyAverage(membersRepublicanParty),                    
					"MostVotedwParty" : 1.73,
                    "LeastVotedwParty": 0,
                    "MostMissedVotes": 0,
                    "LestMissedVotes": 0,
...
/* La keyword return dentro de una función hace que no solo se ejecute el código, sino que la llamada de la función te devuelve un valor*/
